...
// folder. Writes public/local-contracts.json for the harness UI and keeps
...
} from '../../src/components/casino-simulator/local-node/artifacts.ts';
import {
  coinflipGameAbi,
  coinflipGameBytecode,
} from '../../src/components/casino-simulator/local-node/coinflip-game-artifact.ts';
import { watchGameContracts } from './game-contracts.ts';
import type { DeployedGame } from './game-contracts.ts';
...
// Typed loosely across the module boundary rather than sharing viem's client
// generics directly.
...
// The Verify Network simulator ships next to local-node under blockchain/.
async function loadLocalVerifyNetwork(): Promise<LocalVerifyNetworkModule> {
  return await import('../verify-network/index.ts');
}

async function probeRpc(url: string): Promise<number | undefined> {
  try {
    const client = createPublicClient({ transport: http(url, { timeout: 1000, retryCount: 0 }) });
...
function resolveHardhatCli(): string {
  ...
  const { bin } = JSON.parse(readFileSync(packageJsonPath, 'utf-8')) as {
    bin?: string | Record<string, string>;
  };
  const relativeBin = typeof bin === 'string' ? bin : bin?.hardhat;
  if (!relativeBin) {
    throw new Error('Could not locate the hardhat CLI in node_modules.');
  }
...
async function ensureChain(url: string): Promise<{ chainId: number; chainProcess?: ChildProcess }> {
  ...
  chainProcess.on('error', (error) => {
    console.error(
      `[local-node] Failed to start the Hardhat node (${error.message}). Run the repo's install, ` +
        'or start your own chain and point RPC_URL at it.',
    );
    process.exit(1);
  });
  ...
  for (;;) {
    ...
    if (chainId !== undefined) {
      return { chainId, chainProcess };
    }
    if (Date.now() > deadline) {
      throw new Error(`Hardhat node did not become ready at ${url}`);
    }
...
async function main() {
  ...
  const deployedPath = resolve(here, '../../public/local-contracts.json');
