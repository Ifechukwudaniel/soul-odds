...
import { generateVrfProof, publicKeyArrayFromPrivateKey } from './ecvrf.ts';
import type { VrfProof } from './ecvrf.ts';
...
export async function signLocalVrfFulfillment({
  ...
  return await privateKeyToAccount(nodePrivateKey).signTypedData({
    domain: {
  ...
  publicClient.watchEvent({
    ...
    onLogs: (logs) => {
      for (const log of logs) {
        ...
        if (getAddress(assigned.request.fulfiller) !== getAddress(nodeAccount.address)) {
          continue;
        }
