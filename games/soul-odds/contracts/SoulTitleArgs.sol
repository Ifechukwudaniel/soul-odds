...
library SoulOddsTitleArgs {
    uint256 internal constant ERA_COUNT =
        uint256(type(SoulEra).max) + 1;
        ...
        returns (bytes memory args)
    {
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            uint256 recordOffset =
                recordsStart + records.length;

            if (recordOffset > type(uint16).max) {
                revert SoulOddsTitleArgs__TooLarge();
            }

            offsets = bytes.concat(
                offsets,
                bytes2(uint16(recordOffset))
            );

            records = bytes.concat(
                records,
                _encodeRecord(configuration)
            );
        }
        ...
        returns (bytes memory record)
    {
        ...
        if (
            configuration.lifespanTotalWeight == 0
        ) {
            revert SoulOddsTitleArgs__InvalidWeights();
        }

        if (
            configuration.maleWeight == 0 &&
            configuration.femaleWeight == 0
        ) {
            revert SoulOddsTitleArgs__InvalidWeights();
        }
        ...
        for (
            ...
            SoulCrime memory crime =
                configuration.crimes[i];

            if (
                crime.selectionWeight == 0 ||
                crime.commitWeight > 10_000
            ) {
                revert SoulOddsTitleArgs__InvalidWeights();
            }
