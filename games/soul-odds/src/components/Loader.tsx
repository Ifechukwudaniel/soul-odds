 ...
 */
export const Loader = (props: { label?: string; hint?: string; ready?: boolean; onExit?: () => void }) => {
    ...
    motion.add(
      ...
      (context) => {
        ...
        if (reduced) {
          ...
          const ticker = gsap.timeline({ repeat: -1 }).call(cycleLore, undefined, `+=${LORE_HOLD_S}`);
          return () => {
            ticker.kill();
          };
        }
    ...
    >
      ...
      <div ref={introRef} className="relative z-10 flex flex-col items-center gap-8 px-6">
        ...
        <div className="flex w-64 flex-col items-center gap-3">
          ...
          <p className="text-[0.7rem] tracking-wide text-[#e8dcc0]/80">{props.label ?? "Loading…"}</p>
          <p ref={loreRef} className={`${serifFont.className} text-center text-[0.7rem] text-[#3fb6a8]`}>
            {LORE_LINES[0]}
          </p>
        </div>
