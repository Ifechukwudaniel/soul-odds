...
export const AvatarPickerModal = (props: { isOpen: boolean; onClose: () => void }) => {
  ...
  const moveSelection = (fromIndex: number, key: string) => {
    ...
    const targetIndex = key === "Home" ? 0 : key === "End" ? AVATARS.length - 1 : delta === undefined ? null : fromIndex + delta;
    if (targetIndex === null) {
      return;
    }
      ...
      <div className="relative flex flex-col gap-5 p-6">
        ...
        <div role="radiogroup" aria-label="Choose your avatar" className="grid grid-cols-3 gap-3">
          {AVATARS.map((avatar, index) => {
            ...
            return (
              ...
              >
                {isEquipped && (
                  <span aria-hidden="true" className="accent-gradient absolute top-1 right-1 flex h-5 w-5 items-center justify-center rounded-full border border-black">
                ...
                {avatar.cost > 0 && (
                  <span className={`flex items-center gap-1 text-[11px] font-bold ${isLocked ? "text-white/40" : "text-[#F5B83D]"}`}>
        ...
        <button
          ...
          className={`btn w-full cursor-pointer rounded-lg py-3 font-bold disabled:cursor-not-allowed ${
            isAlreadyEquipped || !canAfford ? "bg-[#A7A7A7] text-black opacity-50" : "accent-gradient text-slate-950"
          }`}
        >
          {equipButtonLabel()}
        </button>
