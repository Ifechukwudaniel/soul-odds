...
export function CurrentUserJumpBar({ user, visible, onJump }: CurrentUserJumpBarProps) {
  ...
  return (
    <button
      type="button"
      onClick={onJump}
      className="fixed bottom-6 left-1/2 z-30 flex -translate-x-1/2 cursor-pointer items-center gap-3 rounded-full bg-[#0B0F1A] px-5 py-3 shadow-lg ring-1 ring-white/10"
    >
