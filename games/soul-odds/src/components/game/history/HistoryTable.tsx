...
const NET_COLOR = (net: number) => (net > 0 ? "text-[#6BA84F]" : net < 0 ? "text-[#B7410E]" : "text-white");

export const HistoryTable = (props: { entries: BetHistoryEntry[]; onSelect: (entry: BetHistoryEntry) => void }) => (
      ...
      <div role="table" className="flex min-w-[26rem] flex-col gap-2">
        ...
        {props.entries.map((entry) => (
          <button
            ...
            className={cn(ROW_GRID, "w-full cursor-pointer rounded-xl bg-white/[0.03] py-3 text-left transition-colors hover:bg-white/[0.07]")}
          >
            <span role="cell" className="flex min-w-0 items-center gap-3">
              ...
              <span className="min-w-0">
                <span className={`${serifFont.className} block truncate font-bold text-[#F3D38F] text-sm`}>{fateLine(entry)}</span>
                <span className="block truncate text-white/50 text-xs">Born {fmtYear(entry.bornYear)}</span>
              </span>
            </span>
            <span role="cell" className="text-white text-sm">
              {entry.bets.length === 0 ? "–" : `${entry.bets.filter((bet) => bet.won).length} / ${entry.bets.length}`}
            </span>
            <span role="cell">
              <span className={cn("inline-flex items-center gap-1 font-[600]", NET_COLOR(entry.roundNet))}>
                <CurrencyCoinIcon width={16} height="16" /> {fmtSigned(entry.roundNet)}
              </span>
            </span>
            <span role="cell" className="text-white/70 text-sm">
