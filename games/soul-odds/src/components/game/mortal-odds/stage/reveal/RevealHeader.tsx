...
const YearStamp = (props: { year: number; label: string }) => (
  <div className="flex flex-col items-center">
    <span className={`${serifFont.className} font-bold text-[#F3D38F] text-xl`}>{fmtYear(props.year)}</span>
    <span className="text-[10px] text-white/50 uppercase tracking-[0.2em]">{props.label}</span>
  </div>
);

/** The reveal's headline: the soul's fate, where it began, and the years that bound its life. */
export const RevealHeader = (props: { fate: string; placeName: string; bornYear: number; deathYear: number; epitaph: string; alive: boolean }) => (
  <header className="flex flex-col items-center gap-1 px-2 py-3 text-center">
    ...
    <h3 className={`${serifFont.className} mt-1 font-bold text-2xl text-white sm:text-3xl`}>{props.fate}</h3>
