 ...
 */
export const BetResultStamp = (props: { results: BetResult[]; skill: number; roundNet: number; currency: string; onDismiss: () => void }) => {
      ...
      >
        ...
        {results.length === 0 ? (
          ...
          <div className="mt-4 flex flex-col divide-y divide-black/10">
            {results.map((r) => (
              <div key={r.marketId} className="flex items-center justify-between gap-2 py-2 first:pt-0 last:pb-0">
                <div className="text-left">
                  <p className="font-semibold text-[#33353D] text-sm">{r.marketLabel}</p>
                  <p className="text-[#33353D]/60 text-xs">
                ...
                <div className="flex shrink-0 items-center gap-2">
                  ...
                  <span className={`font-bold text-sm ${r.net >= 0 ? "text-[#6BA84F]" : "text-[#B7410E]"}`}>
                    {r.net >= 0 ? "+" : "−"}
        ...
        {results.length > 0 && (
          <div className="mt-4 flex justify-center border-black/10 border-t pt-3">
        ...
        <div className="mt-4 border-black/10 border-t pt-4 text-center">
          <p className="text-[#33353D]/60 text-xs uppercase tracking-[0.15em]">This round</p>
          <p className={`font-black text-3xl ${roundNet >= 0 ? "text-[#3FB6A8]" : "text-[#B7410E]"}`}>
            {roundNet >= 0 ? "+" : "−"}
            {Math.abs(roundNet).toFixed(2)} <span className="text-lg">{currency}</span>
          </p>
        </div>

        <p className="mt-4 text-center text-[#33353D]/40 text-[11px]">Tap to continue</p>
      </motion.div>
    </motion.div>
  );
};
