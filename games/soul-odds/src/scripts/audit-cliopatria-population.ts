...
const RESEARCHED_FILE = path.join(
  process.cwd(),
  'cliopatria.geojson',
  'cliopatria-population.researched.json',
);
...
function readReferences(): Reference[] {
  const { rows } = JSON.parse(fs.readFileSync(RESEARCHED_FILE, 'utf8')) as {
    rows: (string | number)[][];
  };
  return rows
    ...
    .map((row) => ({
      name: String(row[0]),
      year: (Number(row[1]) + Number(row[2])) / 2,
      population: Number(row[6]),
      areaKm2: Number(row[5]),
    }));
}
...
function checkRules(): string[] {
  ...
  for (const year of YEARS) {
    ...
    for (const e of estimates) {
      const problem = checkPopulation({
        estimate: { low: e.low, mid: e.population, high: e.high },
        areaKm2: e.areaKm2,
        world: world(year),
        year,
      });
    ...
    if (total > world(year) * MAX_OVER_WORLD)
      failures.push(
        `${year}: polities add up to ${(total / 1e6).toFixed(0)}M, over ${MAX_OVER_WORLD}x the ${(world(year) / 1e6).toFixed(0)}M alive`,
      );
  }
  return failures;
}
...
function compareToResearched() {
  const rows = readReferences().flatMap((ref) => {
    ...
    const usable =
      ref.population > 0 &&
      checkPopulation({
        estimate: { low: ref.population, mid: ref.population, high: ref.population },
        areaKm2: ref.areaKm2,
        world: world(ref.year),
        year: ref.year,
      }) === null;
    return estimate && usable
      ? [{ ...ref, estimate: estimate.population, ratio: estimate.population / ref.population }]
      : [];
  });
  const within = (factor: number) =>
    rows.filter((row) => row.ratio >= 1 / factor && row.ratio <= factor).length / rows.length;
  console.log(`\nAgainst ${rows.length} usable researched (Seshat) figures: estimate / researched`);
  console.log(
    `  median ${median(rows.map((row) => row.ratio)).toFixed(2)}x   within 1.5x: ${(within(1.5) * 100).toFixed(0)}%   within 2x: ${(within(2) * 100).toFixed(0)}%   within 5x: ${(within(5) * 100).toFixed(0)}%`,
  );
  const byError = [...rows].sort(
  ...
  for (const row of byError.slice(0, WORST_SHOWN)) {
    console.log(
      `    ${row.name} (${Math.round(row.year)}): ${(row.estimate / 1e6).toFixed(2)}M vs researched ${(row.population / 1e6).toFixed(2)}M  (${row.ratio.toFixed(2)}x)`,
    );
  }
}
