...
const COLUMNS = ["name", "fromYear", "toYear", "wikidata", "seshatId", "areaKm2", "population", "low", "high", "method"] as const;
type Method = "seshat" | "seshat_scaled" | "llm";
type Row = { name: string; fromYear: number; toYear: number; wikidata: string; seshatId: string; areaKm2: number; population: number; low: number; high: number; method: string };
type Researched = { fromYear: number; toYear: number; areaKm2: number; population: number; low: number; high: number };
...
function writeRows(rows: Row[]) {
  ...
  fs.writeFileSync(OUT_FILE, JSON.stringify({ columns: COLUMNS, rows: sorted.map((row) => COLUMNS.map((column) => row[column])) }));
}
...
const gapTo = (row: Researched, year: number) => (year < row.fromYear ? row.fromYear - year : year > row.toYear ? year - row.toYear : 0);
...
function nearest(rows: Researched[] | undefined, year: number): Researched | undefined {
  return rows?.reduce<Researched | undefined>((best, row) => (!best || gapTo(row, year) < gapTo(best, year) ? row : best), undefined);
}

/** A researched figure's density applied to this window's territory. */
function scaledFrom(reference: Researched, job: WindowJob): Pick<Row, "population" | "low" | "high" | "method"> {
  const factor = job.areaKm2 / reference.areaKm2;
  const exact = reference.areaKm2 === job.areaKm2;
  return { population: Math.round(reference.population * factor), low: Math.round(reference.low * factor), high: Math.round(reference.high * factor), method: exact ? "seshat" : "seshat_scaled" };
}

/** The years in the window the polity actually existed, so a row never counts a polity outside its own lifetime. */
const lifetimeIn = (job: WindowJob) => ({ fromYear: Math.max(job.window.fromYear, job.place.fromYear), toYear: Math.min(job.window.toYear, job.place.toYear) });

const rowFor = (job: WindowJob, figures: Pick<Row, "population" | "low" | "high" | "method">): Row => ({
  name: job.name,
  ...lifetimeIn(job),
...
async function main() {
  ...
  for (const job of jobs) {
    const overlapping = researched.get(job.name)?.filter((row) => row.fromYear <= job.window.toYear && row.toYear >= job.window.fromYear);
  ...
  console.log(`${jobs.length} windows across ${new Set(jobs.map((job) => job.name)).size} polities: ${direct.size} from researched figures, ${jobs.length - direct.size} for a model. ${jobs.length - remaining.length} already done, ${remaining.length} to do.`);
                ...
                ...(await generatePopulationEstimate({
                  ...
                  reference:
                    anchor && gapTo(anchor, job.year) <= MAX_REFERENCE_GAP_YEARS
                      ? { year: (anchor.fromYear + anchor.toYear) / 2, population: anchor.population, areaKm2: anchor.areaKm2 }
                      : undefined,
                })),
                method: "llm" satisfies Method,
              };
        const row = rowFor(job, "mid" in figures ? { population: figures.mid, low: figures.low, high: figures.high, method: figures.method } : figures);
  ...
  const calibrated = calibrateToWorld([...done.values()], (year) => interpolate({ points: worldPopCurve, x: year }));
  done.clear();
  for (const row of calibrated) done.set(keyOf(row.name, row.fromYear), row);
  save();

  console.log(`Done. ${remaining.length - failed} windows estimated, ${failed} failed. Wrote ${OUT_FILE}`);
  for (const year of [-3000, -1000, 1, 500, 1000, 1500, 1800, 1900, 2000]) {
    const total = calibrated.filter((row) => !row.name.startsWith("(") && row.fromYear <= year && year <= row.toYear).reduce((sum, row) => sum + row.population, 0);
    console.log(`  year ${year}: rows sum to ${(total / 1e6).toFixed(0)}M (world ${(interpolate({ points: worldPopCurve, x: year }) / 1e6).toFixed(0)}M)`);
  }
}
