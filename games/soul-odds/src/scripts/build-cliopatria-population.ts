...
function distance(a: Row, b: Row): number {
  ...
  return Math.hypot((a.midYear - b.midYear) / YEARS_PER_UNIT, (a.lat - b.lat) / DEGREES_PER_UNIT, ((a.lon - b.lon) * cosLat) / DEGREES_PER_UNIT);
}
...
function applySeshat(rows: Row[], observations: Map<string, Observation[]>) {
  for (const row of rows) {
    ...
    const gapOf = (year: number) => (year < row.fromYear ? row.fromYear - year : year > row.toYear ? year - row.toYear : 0);
    const nearest = candidates.reduce((best, candidate) => (gapOf(candidate.year) < gapOf(best.year) ? candidate : best));
    ...
    const gapTo = (r: Row) => (nearest.year < r.fromYear ? r.fromYear - nearest.year : nearest.year > r.toYear ? nearest.year - r.toYear : 0);
    const anchor = rows
      .filter((other) => other.seshatId === row.seshatId)
      .reduce((best, other) => (gapTo(other) < gapTo(best) || (gapTo(other) === gapTo(best) && other.area > best.area) ? other : best));
    if (anchor.area <= 0) continue;

    row.density = { low: nearest.low / anchor.area, mid: midpoint(nearest.low, nearest.high) / anchor.area, high: nearest.high / anchor.area };
...
function calibrateToWorldPopulation(rows: Row[]) {
  ...
  for (const row of rows) {
    ...
    const total = counted.filter((other) => other.fromYear <= row.midYear && row.midYear <= other.toYear).reduce((sum, other) => sum + other.density!.mid * other.area, 0);
    const world = interpolate({ points: worldPopCurve, x: row.midYear });
    const factor = total > world ? world / total : 1;
    row.density = { low: row.density!.low * factor, mid: row.density!.mid * factor, high: row.density!.high * factor };
  }
}

const csvCell = (value: string | number) => (typeof value === "string" && /[",\n]/.test(value) ? `"${value.replaceAll('"', '""')}"` : String(value));
...
async function main() {
  ...
  const rows: Row[] = features.map((feature) => {
    ...
    return { name: Name, fromYear: FromYear, toYear: ToYear, midYear: midpoint(FromYear, ToYear), wikidata: Wikidata, seshatId: SeshatID, area: Area, lat, lon, gap: 0 };
  });
  ...
  const header = "Name,FromYear,ToYear,Wikidata,SeshatID,AreaKm2,Population,PopulationLow,PopulationHigh,DensityPerKm2,Method,SourceYearGap";
  ...
  await writeFile(
    JSON_PATH,
    JSON.stringify({
      columns: ["name", "fromYear", "toYear", "wikidata", "seshatId", "areaKm2", "population", "low", "high", "method"],
      rows: rows.map((row) => [row.name, row.fromYear, row.toYear, row.wikidata, row.seshatId, Math.round(row.area), Math.round(row.density!.mid * row.area), Math.round(row.density!.low * row.area), Math.round(row.density!.high * row.area), row.method]),
    }),
  );

  const counts = rows.reduce<Record<string, number>>((acc, row) => ({ ...acc, [row.method!]: (acc[row.method!] ?? 0) + 1 }), {});
  ...
  for (const year of [-3000, -1000, -500, 1, 500, 1000, 1500, 1800, 1900, 2000]) {
    const total = rows.filter((row) => row.fromYear <= year && year <= row.toYear).reduce((sum, row) => sum + row.density!.mid * row.area, 0);
