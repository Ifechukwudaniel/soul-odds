...
const STATUS_SEALS: Record<NotificationStatus, { badge: string; icon: React.ReactNode }> = {
  ...
  info: { badge: "bg-(--anubis-faience)", icon: <InformationCircleIcon className="h-5 w-5 text-[#f3ead2]" /> },
  warning: { badge: "bg-(--anubis-gold)", icon: <ExclamationTriangleIcon className="h-5 w-5 text-[#33353D]" /> },
  error: { badge: "bg-(--anubis-carnelian)", icon: <ExclamationCircleIcon className="h-5 w-5 text-[#f3ead2]" /> },
  ...
  return ({ closeToast }: ToastContentProps) => {
    ...
    return (
      <div className="flex w-full items-center gap-3">
        <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full border-2 border-black/30 shadow ${seal.badge}`}>
          {icon ? icon : seal.icon}
        </span>
        <div className="flex-1 break-words font-semibold text-[#33353D] text-xs">{content}</div>
        <button
          type="button"
          onClick={() => closeToast()}
...
function fire(status: NotificationStatus, content: React.ReactNode, options?: NotificationOptions) {
  ...
  if (status === "loading") {
    return toast.loading(bannerRow(status, content, options?.icon), { position, icon: false, closeButton: false, hideProgressBar: true });
  }
...
export const notification = {
  success: (content: React.ReactNode, options?: NotificationOptions) => fire("success", content, options),
  info: (content: React.ReactNode, options?: NotificationOptions) => fire("info", content, options),
  warning: (content: React.ReactNode, options?: NotificationOptions) => fire("warning", content, options),
  error: (content: React.ReactNode, options?: NotificationOptions) => fire("error", content, options),
  loading: (content: React.ReactNode, options?: NotificationOptions) => fire("loading", content, options),
