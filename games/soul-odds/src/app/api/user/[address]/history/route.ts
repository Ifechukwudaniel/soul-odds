...
const recordSchema = z.object({ entries: z.array(betHistoryEntrySchema).min(1).max(MAX_UPLOAD_ENTRIES) });
const patchSchema = z.object({ id: z.string().min(1), story: z.string().min(1), name: z.string().max(60).nullable() });
...
export async function PATCH(request: Request, props: { params: Promise<{ address: string }> }) {
  ...
  if (!body.success) {
    return NextResponse.json({ message: 'Missing or invalid "id", "story" or "name".' }, { status: 400 });
  }
