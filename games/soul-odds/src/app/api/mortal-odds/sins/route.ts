...
export async function GET(request: NextRequest) {
  ...
  const nearest = place ? await findNearestCliopatriaPlace({ lat: place.lat, lon: place.lon, year }) : undefined;
  const own = await findSinVariantsCovering(location, year);
  const covering = own.length > 0 ? own : nearest ? await findSinVariantsCovering(nearest.name, year) : [];

  if (covering.length === 0) {
    return NextResponse.json({ message: "No sin narratives for this place and year." }, { status: 404 });
  }
