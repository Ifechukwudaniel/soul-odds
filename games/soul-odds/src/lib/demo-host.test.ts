...
describe("connectDemoHost persistence", () => {
  it("keeps an open round and its escrowed wager across a reload", async () => {
    ...
    const row = second.latest.snapshot?.sessions.items.find((item) => item.sessionKey === sessionKey);
    expect(row?.isSettled).toBe(false);
    expect(BigInt(second.latest.snapshot?.balances.smartVaultBalance ?? "0")).toBe(990n * 10n ** 18n);
  });
  ...
  it("keeps a settled round, its result and the payout across a reload", async () => {
    ...
    await vi.runAllTimersAsync();
    const { sessionKey } = await opening;
    const sessionId = first.latest.snapshot?.sessions.items.find((item) => item.sessionKey === sessionKey)?.sessionId ?? "";
    ...
    expect(second.latest.snapshot?.balances.smartVaultBalance).toBe(first.latest.snapshot?.balances.smartVaultBalance);
  });
