...
export function connectDemoHost(publish: (snapshot: HostSnapshotV1) => void): Promise<HostApiV1> {
  ...
  const hostApi: HostApiV1 = {
    async openSession(input) {
      ...
      if (wager <= 0n || wager > balance) throw new Error("Not enough demo balance for that wager.");
    ...
    async submitAction(input) {
      ...
      if (session?.phase !== SessionPhase.WAITING_PLAYER_ACTION) throw new Error("This round is no longer open.");
