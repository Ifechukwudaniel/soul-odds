...
export function sinFromNarrative(category: SinCategoryId, narrative: { label: string; phrase: string }, year: number): Sin {
...
export function narrativeFor(narratives: SinNarratives | null, id: string): { label: string; phrase: string } | undefined {
  if (!narratives) return undefined;
  if (id === "violence" || id === "deceit" || id === "greed" || id === "heresy") return narratives[id];
  return undefined;
}
