...
import {
  ...
  reelTickOffsets,
  SETTLE_ROLL_MS,
} from '@/lib/mortal-odds/spin-timeline';
import type { SpinEvent } from '@/lib/mortal-odds/spin-timeline';

const build = (targetYear: number, seed = 1, introMs?: number) =>
  buildSpinTimeline({ targetYear, minYear: -12000, maxYear: 2026, rng: mulberry32(seed), introMs });
const years = (events: SpinEvent[]) =>
  events.flatMap((event) => (event.type === 'year' ? [event] : []));
...
describe('buildSpinTimeline', () => {
  ...
  it('orders events in time and locks after the landing roll settles', () => {
    ...
    expect(
      events.every((event, index) => index === 0 || event.atMs >= (events[index - 1]?.atMs ?? 0)),
    ).toBe(true);
