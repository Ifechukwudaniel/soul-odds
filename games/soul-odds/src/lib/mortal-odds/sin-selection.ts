...
export function crimeMaskOfSinOption(optionId: string): number {
  return categoriesOfSinOption(optionId).reduce((mask, id) => mask | (1 << CATEGORY_IDS.indexOf(id)), 0);
}

export function categoriesOfCrimeMask(crimeMask: number): SinCategoryId[] {
  return SIN_CATEGORIES.filter((_category, bit) => (crimeMask & (1 << bit)) !== 0).map((category) => category.id);
}
