...
const CAUSES: readonly Cause[] = [
  ...
  { label: (sin) => `Hanged before a jeering crowd for ${sin}`, ages: [14, 80], to: 1970, sinOnly: true, weight: 6 },
  { label: (sin) => `Beheaded in the public square for ${sin}`, ages: [14, 80], to: 1800, sinOnly: true, weight: 4 },
  { label: (sin) => `Flogged to death for ${sin}`, ages: [14, 80], to: 1850, sinOnly: true, weight: 3 },
  { label: (sin) => `Executed in prison for ${sin}`, ages: [18, 80], from: 1900, sinOnly: true, weight: 6 },
  { label: (sin) => `Beaten to death by an angry mob over ${sin}`, ages: [14, 80], sinOnly: true, weight: 4 },
  { label: () => "Died in a foul cell, despised and forgotten", ages: [16, 90], sinOnly: true, weight: 5 },
  { label: () => "Died in disgrace, shunned by kin and neighbours", ages: [16, 100], sinOnly: true, weight: 6 },
];
 ...
 */
export function pickCauseOfDeath(options: { life: Life; currentYear: number; rng: Rng }): string | null {
