...
const WAGER = 10n ** 19n;
const bet = (marketId: string, optionId: string): Bet => ({
  marketId,
  kind: 'choice',
  optionId,
  stake: 1,
...
describe('previewSinsPrices', () => {
  ...
  it("pins to one era's odds instead of averaging every era", () => {
    const perEra = soulOddsConfigurations.map(
      (_configuration, index) => previewSinsPrices(WAGER, 2, index).heresy!.odds!,
    );
...
describe('previewCategoryPrices', () => {
  it('prices a 5 to 29 death near 8x before the era is known, and no era pays a silly amount for it', () => {
    ...
    const perEra = soulOddsConfigurations.map(
      (_configuration, index) => previewCategoryPrices(WAGER, 0, index).age.y!.odds!,
    );
...
describe('configurationIndexFromGameState', () => {
  it('reads the era index the contract stored', () => {
    expect(configurationIndexFromGameState(encodeAbiParameters([{ type: 'uint256' }], [3n]))).toBe(
      3,
    );
  });
  ...
  it("returns null for an empty state or an index past the title's eras", () => {
    ...
    expect(
      configurationIndexFromGameState(encodeAbiParameters([{ type: 'uint256' }], [99n])),
    ).toBeNull();
  });
});
