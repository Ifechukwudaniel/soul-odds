...
const ARCHAIC = /\b(bows?|arrows?|archer\w*|swords?|spears?|lances?|catapults?|chariots?|encampments?|armou?r|shields?|crossbows?)\b/i;

/** The first archaic word a story uses despite being set in a modern year, or null when it reads true to the era. */
export function findAnachronism(options: { story: string; year: number; deathYear: number }): string | null {
