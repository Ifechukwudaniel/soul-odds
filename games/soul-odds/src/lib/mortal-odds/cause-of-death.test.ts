...
const causesOver = (subject: Life) =>
  new Set(
    Array.from({ length: 300 }, (_, seed) =>
      pickCauseOfDeath({ life: subject, currentYear: CURRENT_YEAR, rng: createRng(seed) }),
    ),
  );
...
describe('pickCauseOfDeath', () => {
  it('returns null while the soul is still living', () => {
    expect(
      pickCauseOfDeath({
        life: life({ year: 2000, age: 40, deathYear: 2040 }),
  ...
  it('returns null when a shock killed the soul', () => {
    ...
    expect(
      pickCauseOfDeath({ life: life({ shock }), currentYear: CURRENT_YEAR, rng: createRng(1) }),
    ).toBeNull();
  });
  ...
  describe('with a sin on record', () => {
    const sin = {
      id: 'greed',
      label: 'Grave Robbery',
      phrase: 'stole from a tomb',
      from: 0,
      to: null,
    };

    it('offers shameful deaths that name the sin', () => {
      expect(causesOver(life({ sin }))).toContain(
        'Hanged before a jeering crowd for grave robbery',
      );
    });
    ...
    it('never offers shameful deaths to a clean soul', () => {
      ...
      expect(
        causes.filter(
          (cause) => cause?.includes('for grave robbery') || cause?.startsWith('Died in disgrace'),
        ),
      ).toEqual([]);
    });
