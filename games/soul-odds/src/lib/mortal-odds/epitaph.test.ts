...
import { ERA_BY_PERIOD, epitaphs, eraOf, lintEpitaph, lintEpitaphs, matchesScenario, pickEpitaph, renderEpitaph, scenarioOf } from "@/lib/mortal-odds/epitaph";
...
const entry = (id: string, when: Epitaph["when"], text = "A quiet line."): Epitaph => ({ id, text, when });

const pick = (l: Life, pool: Epitaph[], seed = 1, recent: string[] = []) =>
  pickEpitaph({ life: l, currentYear: CURRENT_YEAR, placeName: "Somewhere", rng: mulberry32(seed), recent, pool });
...
describe("matchesScenario", () => {
  ...
  it("requires every named field to match", () => {
    ...
    expect(matchesScenario(entry("b", { sex: ["girl"], ageBand: ["elder"] }), scenario)).toBe(false);
  });
...
describe("pickEpitaph", () => {
  it("never picks an entry whose conditions the life fails", () => {
    const pool = [entry("boy-only", { sex: ["boy"] }), entry("girl-only", { sex: ["girl"] }), entry("elder-only", { ageBand: ["elder"] })];
  ...
  it("favours the more specific of two matching entries", () => {
    const pool = [entry("generic", {}), entry("specific", { sex: ["girl"], ageBand: ["adult"], setting: ["land"] })];
    let specific = 0;
...
describe("renderEpitaph", () => {
  it("fills pronouns for a girl", () => {
    expect(renderEpitaph("{He} lost {his} way; the road took {him}.", life({ sex: "girl" }), "Ur")).toBe("She lost her way; the road took her.");
  });

  it("fills pronouns for a boy", () => {
    expect(renderEpitaph("{He} lost {his} way; the road took {him}.", life({ sex: "boy" }), "Ur")).toBe("He lost his way; the road took him.");
  });

  it("fills years, place and lifespan", () => {
    const text = renderEpitaph("{born} in {place}, gone in {died}, after {lifespan}.", life({ year: 1500, deathYear: 1540, age: 40 }), "Ur");
  ...
  it("capitalizes a sentence-initial cause and sin", () => {
    ...
    expect(renderEpitaph("{Cause} came; {sin} and {Sin}; {he} {sinPhrase}.", life({ shock, sin }), "Ur")).toBe(
      "The Black Death came; theft and Theft; she stole to get by.",
    );
  });
...
describe("lintEpitaph", () => {
  ...
  it("requires tags for wording that only fits some lives", () => {
    ...
    expect(problemsFor("Never learned to read, but knew the fields.", { literacy: ["illiterate"], setting: ["land"] })).toEqual([]);
  });

  it("flags an entry no life can match", () => {
    expect(problemsFor("A quiet ending, overall.", { status: ["alive"], cause: ["catastrophe"] })).toHaveLength(1);
  });
});
  ...
  it("gives every status and age band at least one line for any life", () => {
    for (const status of ["alive", "dead"]) {
      for (const age of [0, 3, 10, 20, 35, 50, 70, 90]) {
        ...
        expect(pick(life({ age, deathYear }), epitaphs).id, `${status} ${age}`).not.toBe("fallback");
      }
    }
  });
});
