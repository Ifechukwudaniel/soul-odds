...
const pick = (year: number, pool: TimeStory[], seed = 1, recent: string[] = []) => pickTimeStory({ year, rng: mulberry32(seed), recent, pool });
...
describe("renderTimeStory", () => {
  it("fills the population placeholder for the year", () => {
    expect(renderTimeStory("About {people} people lived then.", 1900)).toMatch(/^About .+ people lived then\.$/);
    expect(renderTimeStory("About {people} people lived then.", 1900)).not.toContain("{");
  });
});

describe("lintTimeStory", () => {
  const problemsFor = (from: number, to: number, text: string) => lintTimeStory({ id: "t", from, to, text });

  it("accepts a plain story", () => {
    expect(problemsFor(1914, 1918, "The world was at war, and millions of young men were sent away from home.")).toEqual([]);
  });

  it("rejects an inverted or out-of-bounds range", () => {
    expect(problemsFor(1950, 1900, "The world was at war, and millions of young men were sent away.")).toHaveLength(1);
    expect(problemsFor(-60000, -50000, "Small bands followed the herds across the frozen plains.")).toHaveLength(1);
    expect(problemsFor(2000, 3000, "The internet reached almost everywhere, and news crossed the planet.")).toHaveLength(1);
  });

  it("rejects digits, unknown placeholders and bad lengths", () => {
    expect(problemsFor(1914, 1918, "The war that began in 1914 spread across the whole wide world.")).toHaveLength(1);
    expect(problemsFor(1914, 1918, "The war spread and {year} became a year nobody forgot.")).toHaveLength(1);
    expect(problemsFor(1914, 1918, "Too short.")).toHaveLength(1);
  });

  it("rejects wording from a later age than the range starts", () => {
    expect(problemsFor(1800, 1849, "The first railways were stitching towns together across the land.")).toHaveLength(1);
    expect(problemsFor(1825, 1849, "The first railways were stitching towns together across the land.")).toEqual([]);
    expect(problemsFor(-50000, -10001, "Coins and cities were unknown, and the herds were all that mattered.")).toHaveLength(2);
  });

  it("does not flag the placeholder itself", () => {
    expect(problemsFor(-50000, -10001, "Only {people} people were alive in the whole wide world.")).toEqual([]);
  });
});

describe("lintTimeStories", () => {
  it("flags year gaps and duplicate ids", () => {
    const list = [story("a", -50000, 1000, "One quiet story about the long early history."), story("a", -50000, 1000, "Another quiet story about the long early history.")];
    ...
    expect(problems.some((problem) => problem.includes("coverage") && problem.includes("1001..1200"))).toBe(true);
  });
});
