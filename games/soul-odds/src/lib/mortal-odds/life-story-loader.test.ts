...
function memoryCache() {
  ...
  return { read: (key: string) => store.get(key) ?? null, write: (key: string, value: LifeStoryPayload) => void store.set(key, value) };
}
...
describe("LifeStoryLoader", () => {
  ...
  describe("summoning another soul before the previous story arrives", () => {
    it("ignores a slow, stale response for the round the player already left", async () => {
      ...
      const fetchStory = vi.fn().mockImplementation((key: string) => (key === "session-old" ? oldRequest.promise : Promise.resolve(newPayload)));
      const loader = new LifeStoryLoader({ cache: memoryCache(), fetchStory });
      const onResolvedOld = vi.fn();
      const onResolvedNew = vi.fn();

      const stateForOld = loader.request("session-old", payload("old soul's local story"), onResolvedOld);
      expect(stateForOld).toEqual({ ready: false });

      // The player summons another soul before the old round's OpenRouter request lands.
      const stateForNew = loader.request("session-new", payload("new soul's local story"), onResolvedNew);
    ...
    it("still caches a stale response under its own session key", async () => {
      ...
      const loader = new LifeStoryLoader({ cache, fetchStory: (key) => (key === "session-old" ? oldRequest.promise : Promise.resolve(newPayload)) });
    ...
    it("resumes applying updates once the player returns to the latest key", async () => {
      ...
      const loader = new LifeStoryLoader({ cache: memoryCache(), fetchStory: () => Promise.resolve(generated) });
      const onResolved = vi.fn();
