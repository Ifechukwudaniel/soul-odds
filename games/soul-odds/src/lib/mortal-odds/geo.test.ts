...
describe('project', () => {
  ...
  it('scales with viewport size', () => {
    expect(project({ lon: 0, lat: 0, viewport: { width: 1000, height: 500 } })).toEqual({
      x: 500,
      y: 250,
    });
  });
...
describe('densityField', () => {
  ...
  it('shifts weight between eras as regional shares change', () => {
    ...
    const sahel = (blobs: typeof stoneAge) =>
      blobs.find((blob) => blob.id === 'the West African Sahel')?.intensity;
