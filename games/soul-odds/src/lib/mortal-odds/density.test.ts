...
describe("countryDensityAt", () => {
  ...
  it("is denser in a later, more populous year", () => {
    expect(countryDensityAt({ country: italy, year: 1800 })).toBeGreaterThan(countryDensityAt({ country: italy, year: 100 }));
  });

  it("gives a year between two time steps a figure between theirs", () => {
    const [before, mid, after] = [1750, 1775, 1800].map((year) => countryDensityAt({ country: italy, year }));
...
describe("populationFromCountries", () => {
  it("adds up each country's density times the land covered there", () => {
    const year = 1500;
    const expected = countryDensityAt({ country: italy, year }) * 100_000 + countryDensityAt({ country: egypt, year }) * 50_000;
    expect(populationFromCountries({ year, landByCountry: { [italy]: 100_000, [egypt]: 50_000 } })).toBeCloseTo(expected);
  });

  it("scales with the land covered", () => {
    const small = populationFromCountries({ year: 1500, landByCountry: { [italy]: 100_000 } });
    expect(populationFromCountries({ year: 1500, landByCountry: { [italy]: 200_000 } })).toBeCloseTo(small * 2);
  });
});
