...
export const reelTickOffsets = (options: { rollMs: number; count: number }) =>
  Array.from(
    ...
    (_, index) => options.rollMs * (1 - Math.sqrt(1 - (index + 1) / options.count)),
  );
 ...
 */
export const buildSpinTimeline = (options: {
  targetYear: number;
  minYear: number;
  maxYear: number;
  rng: Rng;
  ...
  const nearMiss = Math.round(
    clamp(
      targetYear + direction * span * NEAR_MISS_SHARE * (0.25 + rng() * 0.75),
      minYear,
      maxYear,
    ),
  );
  const hopYears = [...randomYears, nearMiss].map((year) =>
    year === targetYear ? (year < maxYear ? year + 1 : year - 1) : year,
  );
  ...
  hopYears.forEach((year, index) => {
    ...
    reelTickOffsets({ rollMs: HOP_ROLL_MS, count: HOP_TICKS }).forEach((offset, step) =>
      events.push({ atMs: atMs + offset, type: 'tick', step }),
    );
  });
  ...
  reelTickOffsets({ rollMs: SETTLE_ROLL_MS, count: SETTLE_TICKS }).forEach((offset, step) =>
    events.push({ atMs: landingAtMs + offset, type: 'tick', step }),
  );
