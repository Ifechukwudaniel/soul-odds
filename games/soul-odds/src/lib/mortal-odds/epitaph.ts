...
export const CONDITION_FIELDS = ["status", "ageBand", "sex", "cause", "literacy", "setting", "sin", "era", "region"] as const;
...
const specificity = (epitaph: Epitaph) => CONDITION_FIELDS.filter((field) => epitaph.when[field] !== undefined).length;
...
function tokensFor(life: Life, placeName: string): Record<TokenName, string> {
  const pronouns = life.sex === "girl" ? { he: "she", his: "her", him: "her" } : { he: "he", his: "his", him: "him" };
  return {
    he: pronouns.he,
...
export function renderEpitaph(text: string, life: Life, placeName: string): string {
  ...
  return text.replace(/\{(\w+)\}/g, (match, name: string) => (isTokenName(name) ? tokens[name] : match));
}

const FALLBACKS = { alive: "The story is not over yet.", dead: "Remembered, if only by the scales." };
...
export function pickEpitaph(options: {
  ...
  const chosen = pickWeighted({ items: candidates, weight: (epitaph) => 2 ** specificity(epitaph), rng });
 ...
 */
const GUARDS: Guard[] = [
  { pattern: /\b(child|children|infant|baby|cradle|newborn|toddler)\b/i, field: "ageBand", allowed: ["infant", "child", "juvenile"] },
  { pattern: /\b(elder|elders|grandmother|grandfather|grandchildren|grey|gray|white hair|old age)\b/i, field: "ageBand", allowed: ["elder", "ancient"] },
  { pattern: /\b(died|dead|buried|grave|tomb|laid to rest)\b/i, field: "status", allowed: ["dead"] },
  { pattern: /\b(plague|pandemic|epidemic|war|wars|famine|battle|soldier|rebellion|invasion|conquest|pestilence)\b/i, field: "cause", allowed: CATASTROPHE_VALUES },
  { pattern: /\b(thief|steal|stole|stolen|murder|murdered|robbed|sin|sins|sinned|guilty|crime|criminal|heresy|heretic|forged|fraud|bribe|bribed|smuggled|adultery|pirate|perjury)\b/i, field: "sin", allowed: SIN_CATEGORY_IDS },
  { pattern: /\b(innocent|blameless|sinless|unstained)\b/i, field: "sin", allowed: ["clean"] },
  { pattern: /\b(city|cities|street|streets|urban|market)\b/i, field: "setting", allowed: ["city"] },
  { pattern: /\b(field|fields|farm|farmer|village|villages|harvest|plough|plow|herd|pasture|countryside|soil)\b/i, field: "setting", allowed: ["land"] },
  { pattern: /\b(scribe|scribes|ink|scroll|scrolls|literate|letters)\b/i, field: "literacy", allowed: ["literate"] },
  { pattern: /\b(unlettered|illiterate|never learned to read|could not read|never read)\b/i, field: "literacy", allowed: ["illiterate"] },
  { pattern: /\b(she|her|hers|herself|girl|daughter|wife|mother|sister|maiden|lady|queen|widow|woman)\b/i, field: "sex", allowed: ["girl"] },
  { pattern: /\b(he|his|him|himself|boy|son|husband|father|brother|lad|lord|king|widower|man)\b/i, field: "sex", allowed: ["boy"] },
];
...
export function lintEpitaph(epitaph: Epitaph): string[] {
  ...
  for (const field of CONDITION_FIELDS) {
    for (const value of epitaph.when[field] ?? []) {
      if (!VOCABULARY[field].includes(value)) problems.push(`${label} when.${field} has unknown value "${value}"`);
    }
  }

  if (epitaph.text.length < MIN_LENGTH || epitaph.text.length > MAX_LENGTH) {
    problems.push(`${label} text must be ${MIN_LENGTH}-${MAX_LENGTH} characters, got ${epitaph.text.length}`);
  }
  if (/\d/.test(epitaph.text)) problems.push(`${label} text contains a digit; use {age}, {born} or {died}`);
  ...
  for (const [, name] of epitaph.text.matchAll(/\{(\w*)\}/g)) {
    ...
    if (rule && !restrictedTo(rule.field, rule.allowed)) {
      problems.push(`${label} uses {${name}} so when.${rule.field} must be limited to [${rule.allowed.join(", ")}]`);
    }
  }
  ...
  for (const guard of GUARDS) {
    ...
    if (hit && !restrictedTo(guard.field, guard.allowed)) {
      problems.push(`${label} text says "${hit}" so when.${guard.field} must be limited to [${guard.allowed.join(", ")}]`);
    }
  }
...
function isBaseline(epitaph: Epitaph): boolean {
  return CONDITION_FIELDS.every((field) => field === "status" || field === "ageBand" || epitaph.when[field] === undefined);
}
...
export function lintEpitaphs(list: Epitaph[]): string[] {
  ...
  for (const status of VOCABULARY.status) {
    for (const ageBand of VOCABULARY.ageBand) {
      const scenario: Scenario = { status: [status], ageBand: [ageBand], sex: [], cause: [], literacy: [], setting: [], sin: [], era: [], region: [] };
      const count = baselines.filter((epitaph) => matchesScenario(epitaph, scenario)).length;
      if (count < MIN_BASELINE) problems.push(`coverage: status "${status}" + ageBand "${ageBand}" has ${count} baseline epitaphs, needs ${MIN_BASELINE}`);
    }
  }
