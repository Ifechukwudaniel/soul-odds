...
import { categoriesOfCrimeMask, categoriesOfSinOption, crimeMaskOfSinOption, sinOptionId, sinsOf } from "@/lib/mortal-odds/sin-selection";
...
describe("crimeMaskOfSinOption", () => {
  ...
  it("round-trips through the mask", () => {
    expect(sinOptionId(categoriesOfCrimeMask(crimeMaskOfSinOption("deceit+heresy")))).toBe("deceit+heresy");
...
describe("sinsOf", () => {
  it("returns every recorded sin", () => {
    expect(sinsOf({ ...LIFE, sin: sin("violence"), sins: [sin("violence"), sin("greed")] })).toHaveLength(2);
  });
