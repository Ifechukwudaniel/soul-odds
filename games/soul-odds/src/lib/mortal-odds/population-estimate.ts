...
function parseLandByCountry(value: unknown): LandByCountry | undefined {
  ...
  return Object.fromEntries(value.split(",").map((part) => part.split(":").map(Number)).flatMap(([country, km2]) => (country !== undefined && km2 !== undefined ? [[country, km2]] : [])));
}

function toRow(values: (string | number)[]): PopulationRow[] {
  const [name, fromYear, toYear, wikidata, seshatId, areaKm2, population, low, high, method, countries] = values;
  if (
    typeof name === "string" &&
    ...
    return [{ name, fromYear, toYear, wikidata, seshatId, areaKm2, population, low, high, method, ...(parseLandByCountry(countries) && { landByCountry: parseLandByCountry(countries) }) }];
  }
  return [];
}
...
function figuresFor(row: PopulationRow, year: number): Pick<PopulationEstimate, "population" | "low" | "high"> {
  if (row.method === "density" && row.landByCountry && row.population > 0) {
    ...
    return { population: Math.round(population), low: Math.round(population * lowRatio), high: Math.round(population * highRatio) };
  }
...
export function estimatePopulation(options: { empire?: string; wikidata?: string; year: number }): PopulationEstimate | null {
  ...
  const candidates = wikidata ? ROWS.filter((row) => row.wikidata === wikidata) : empire ? rowsNamed(empire, year) : [];
