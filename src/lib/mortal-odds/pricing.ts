...
import type { ChanceTag, MarketConfig, MarketPrices, Price } from "@/types";
...
/** Share of bookie samples whose death year falls within `window` of `guess`. */
export function deathYearP(options: { samples: BookieLife[]; guess: number; window: number }): number {
...
export function medianDeathYear(samples: BookieLife[]): number {
