...
describe("applyShocks", () => {
  it("never strikes a life that never overlaps the shock's window", () => {
    const hit = applyShocks({ year: 2000, region: "eur", sex: "boy", age: 10, rng: mulberry32(1), shocks: [WAR] });
    expect(hit).toBeNull();
  });

  it("never strikes a region the shock doesn't list", () => {
    const hit = applyShocks({ year: 1940, region: "eas", sex: "boy", age: 10, rng: mulberry32(1), shocks: [WAR] });
    expect(hit).toBeNull();
  });

  it("strikes within the exposure window when the rate is certain", () => {
    const hit = applyShocks({ year: 1940, region: "eur", sex: "boy", age: 10, rng: mulberry32(1), shocks: [WAR] });
  ...
  it("respects an age window filter", () => {
    ...
    const hit = applyShocks({ year: 1940, region: "eur", sex: "boy", age: 10, rng: mulberry32(1), shocks: [narrow] });
    expect(hit).toBeNull();
  });

  it("applies the sex multiplier", () => {
    const sexSkewed: ShockConfig = { ...WAR, rate: { eur: 0.5 }, sexMultiplier: { boy: 2, girl: 0 } };
    const girlHit = applyShocks({ year: 1940, region: "eur", sex: "girl", age: 10, rng: mulberry32(1), shocks: [sexSkewed] });
    expect(girlHit).toBeNull();
  });

  it("is deterministic for a fixed seed", () => {
    const a = applyShocks({ year: 1940, region: "eur", sex: "boy", age: 10, rng: mulberry32(42), shocks: [WAR] });
    const b = applyShocks({ year: 1940, region: "eur", sex: "boy", age: 10, rng: mulberry32(42), shocks: [WAR] });
    expect(a).toEqual(b);
  });
});
