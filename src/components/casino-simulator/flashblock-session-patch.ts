...
export function applyFlashblockEvent(
  ...
  switch (event.eventName) {
    ...
    case 'CasinoSessionAdvanced': {
      ...
      if (current.step !== undefined && event.step <= current.step) {
        return patches;
      }
      next = {
        ...current,
...
export function pruneFlashblockPatches(
  ...
  const stale = Object.values(patches).filter((patch) => {
    ...
    if (patch.step !== undefined && (row.step ?? -1) < patch.step) {
      return false;
    }
    if (patch.settled && row.status !== 'settled' && !isTerminalPhase(row.phase)) {
      return false;
    }
