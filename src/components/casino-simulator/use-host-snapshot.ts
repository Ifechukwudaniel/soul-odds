...
import type { CasinoGameManifestV1, HostSnapshotV1, RandomnessRequestV1 } from '@/libs/casino-sdk';
import { isTerminalPhase, phaseName } from './casino';
import type { CasinoSessionRow, GameIntegration } from './casino';
...
import type { FlashblockSessionPatches } from './flashblock-session-patch';
import {
  reconcileMonotonicSessionItems,
  sortSessionItemsNewestFirst,
} from './monotonic-session-items';
import { mergeCasinoSessionItems } from './optimistic-casino-session';
import type { CasinoSessionItem, OptimisticCasinoSession } from './optimistic-casino-session';
...
function maskMidRoundWaiting(
  ...
  return items.map((item) => {
    ...
    if (item.phase === 1) {
      return lastActionableItems.get(item.sessionId) ?? item;
    }
    return item;
  });
}
