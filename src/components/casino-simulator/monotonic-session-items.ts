import { isTerminalPhase } from './casino';
...
function isCompleteSettledItem(item: CasinoSessionItem): boolean {
  if (!item.isSettled || item.payout === undefined) {
    return false;
  }
...
export function reconcileMonotonicSessionItems(
  ...
  const reconciled = items.map((item) => {
    ...
    if (!isCompleteSettledItem(next)) {
      ...
      if (next.isSettled || isTerminalPhase(next.phase)) {
        next = demoteToWaitingRandomness(next);
      }
    }
