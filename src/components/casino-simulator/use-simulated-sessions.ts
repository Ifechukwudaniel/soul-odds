...
import { applyFlashblockEvent, pruneFlashblockPatches } from './flashblock-session-patch';
...
export function useStuckSessionId(
  ...
  useEffect(
    () =>
      runtime.publicClient.watchBlockNumber({
        poll: true,
  ...
  return useMemo(() => {
    if (currentBlock === undefined || !sessionRows) {
      return;
    }
