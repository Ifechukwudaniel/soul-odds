...
export type SimulatorConfig = {
  /** Origin the game iframe is served from, e.g. http://localhost:3000/casino/coinflip/ */
  gameUrl: string;
  rpcUrl: string;
...
export const DEFAULT_CONFIG: SimulatorConfig = {
  gameUrl: 'http://localhost:3000/casino/coinflip/',
...
/**
 * Reads public/local-contracts.json (written by `bun run blockchain:node`).
 * Absent (404) until the local node has deployed — the panel then relies on
...
export async function fetchLocalDeployedContracts(): Promise<LocalDeployedContracts | undefined> {
  try {
    ...
    if (!raw.host || !raw.vault || !raw.token) {
      return undefined;
    }
    return {
      host: raw.host,
