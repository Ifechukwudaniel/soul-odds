...
import type { CasinoGameManifestV1, HostApiV1, HostSnapshotV1 } from '@/libs/casino-sdk';
import {
  betPlaced,
  refundStake,
...
import type { BatchAction, EncodedSessionSnapshot, GameIntegration } from './casino';
...
import type { OptimisticCasinoSession } from './optimistic-casino-session';
...
export function useGameHost(
  ...
  const [rawBalance, setRawBalance] = useState<bigint | undefined>();
  useEffect(() => {
  ...
  const rememberSessionSnapshots = useCallback((receipt: TransactionReceipt) => {
    ...
    for (const [sessionId, snapshot] of parseAdvancedSessionSnapshots(receipt)) {
      ...
      if (known && known.step >= snapshot.step) {
        continue;
      }
  ...
  const requireReadyWallet = useCallback(() => {
    if (walletStatus === 'disconnected') {
      throw new Error('Connect a wallet to play.');
    }
    if (walletStatus !== 'ready') {
      throw new Error('Wallet setup is not complete.');
    }
  }, [walletStatus]);
  ...
  const methods = useMemo<HostApiV1>(
    () => ({
      ...
      async getRandomnessVerification({ sessionId }) {
        ...
        if (!indexedRow) {
          throw new Error(`Session ${sessionId} not in the indexed feed (yet).`);
        }
        const row = mergeFlashblockPatchIntoRow(indexedRow, patch);
        return await verifySessionRandomness({
