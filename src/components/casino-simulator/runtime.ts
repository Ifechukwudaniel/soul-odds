...
import type { Address, Chain, Hex, PublicClient, TransactionReceipt, WalletClient } from 'viem';
...
import { createSessionEventFeed } from './session-events';
import type { SessionEventFeed } from './session-events';
...
export async function createSimulatorRuntime(
  ...
  const unsubscribeIndexed = feed.subscribeIndexed((event) => {
  ...
  const sendActions: SimulatorRuntime['sendActions'] = async (actions, onAccepted) => {
    if (actions.length === 0) {
