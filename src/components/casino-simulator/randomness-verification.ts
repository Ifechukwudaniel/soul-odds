...
function verifyEcvrf(input: {
  ...
  try {
    ...
    let betaValue = 0n;
    for (const b of beta) {
      betaValue = (betaValue << 8n) | BigInt(b);
    }
...
function findFulfillmentLog(
  ...
  for (const log of logs) {
    if (!sameAddress(log.address, router) || log.topics.length === 0) {
      continue;
    }
    try {
...
async function verifyRequest(
  ...
  const { requestId } = base;
  let { transactionHash } = base;
  if (!transactionHash) {
    // Marked fulfilled without a hash (row projected before the field
    // existed): the router's log carries it.
    const logs = await publicClient.getLogs({
      address: router,
      event: routerAbi.find(
        (item) => item.type === 'event' && item.name === 'RandomnessFulfilled',
      )!,
      args: { requestId },
      fromBlock: 0n,
    });
