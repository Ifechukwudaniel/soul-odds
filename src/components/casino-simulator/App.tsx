...
import { DEFAULT_CONFIG, fetchLocalDeployedContracts, loadConfig, saveConfig } from './config';
import type { LocalDeployedContracts, SimulatorConfig } from './config';
...
export function App() {
  ...
  const [detected, setDetected] = useState<LocalDeployedContracts | undefined>();
  const [appliedConfig, setAppliedConfig] = useState<SimulatorConfig | undefined>();
  const [runtime, setRuntime] = useState<SimulatorRuntime | undefined>();
  ...
  useEffect(() => {
    ...
    return () => {
      mq.removeEventListener('change', collapseOnPhone);
    };
  }, []);
  ...
  const integration = useMemo<GameIntegration | undefined>(() => {
    if (!appliedConfig?.gameAddress || !appliedConfig.gameUrl) {
      return;
    }
    return {
