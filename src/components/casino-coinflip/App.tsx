...
import { computeMaxWager } from '@/libs/casino-sdk/guest';
...
import type { StagePhase } from './components/CoinStage';
...
const backdrop = '/casino/coinflip/backdrop.webp';
const backdropMobile = '/casino/coinflip/backdrop-mobile.webp';
...
export function App() {
  ...
  useEffect(() => {
    if (!round || round.status !== 'waiting' || !snapshot) {
      return;
    }
    ...
    const rawRandomness = row.raw.randomness === undefined ? 0n : BigInt(row.raw.randomness);
    const outcome =
      (row.raw.gameState ? decodeGameState(row.raw.gameState) : null) ??
      (rawRandomness === 0n ? null : outcomeFromRandomness(round.bet, rawRandomness)) ??
      (row.payout === undefined ? null : outcomeFromResult(round.bet, BigInt(row.payout) > 0n));
    if (!outcome) {
      return;
    } // result not synced yet — wait for the next push
    ...
    const rowPayout = row.payout === undefined ? 0n : BigInt(row.payout);
    setRound((current) =>
  ...
  useEffect(() => {
    ...
    if (round.status !== 'landing' && round.status !== 'done') {
      return;
    }
  ...
  const openRound = useCallback(
    async (bet: CoinflipBet, wager: bigint) => {
      ...
      try {
        ...
        setRound(null);
        setError(error instanceof Error ? error.message : 'Failed to open the round.');
      }
    },
    [hostApi],
  );
  ...
  const winMultiplier =
    round && round.payout !== undefined && round.wager > 0n
      ? `${(Number((round.payout * 10_000n) / round.wager) / 10_000).toFixed(2)}x`
      : '0.00x';
  const winNet =
