...
export const canonicalCasinoGameId = (value: string | undefined | null): string => {
  ...
  return trimmed.toLowerCase().replaceAll(/[^a-z0-9]/g, '');
};
...
export const resolveManifestMetadata = (
  ...
  const locale =
    manifest.locales[requestedLocale] === undefined ? manifest.defaultLocale : requestedLocale;
  // `validateCasinoGameManifest` rejects any manifest whose defaultLocale entry
  // is missing, so this fallback is guaranteed to resolve.
