...
| `slot-engine/`                                | Optional add-on for slots: shared engine contract, title compiler/deployer CLI, Stake Engine importer and the Lucky Reels example. See `docs/SLOT_ENGINE.md`.                                                                                                                        |
...
This package is a workspace root (bun shown; npm, pnpm and yarn work the same). One install covers the simulator,
the local VRF node, the slot engine and the example game; one command runs the whole local stack
(chain + VRF node + casino deployment + simulator harness + example game):
