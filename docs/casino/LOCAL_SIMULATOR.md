...
The app ships a fully local test environment: a harness UI at `/casino-simulator`
(`src/components/casino-simulator/`) that mounts any game iframe by URL and drives it through the
exact same `casino-sdk` bridge (`src/libs/casino-sdk/`) as the production Chain.wtf host, plus a
one-command local backend — its own chain, a minimal casino deployment, and a real Verify Network
VRF node. Everything runs on your machine; no Chain.wtf account or backend access is needed.
...
```sh
bun install
bun run blockchain:local   # local chain + VRF node + deployment
bun run dev                # the app, including the harness at /casino-simulator
```

Open **http://localhost:3000/casino-simulator** — the harness polls for the local deployment,
fills the setup panel and starts itself as soon as the chain is up.

The player defaults to dev-mnemonic account #0, which holds 1,000,000 test chUSD, and the game URL
defaults to `http://localhost:3000/casino/coinflip/` (the coinflip example, also part of this
app). Point it at your own game in the collapsible setup panel — an unregistered game contract is
registered on the local host automatically under the configured name. Query params override the
saved setup:

```
http://localhost:3000/casino-simulator?game=http://localhost:5173&gameAddress=0x…&rpc=…
```

## What the local backend runs

`bun run blockchain:node` starts and wires up:
...
4. **Funding** — player tokens, vault liquidity and the VRF router client balance are topped up,
   and the deployment is written to `public/local-contracts.json`, which the harness fetches
   directly.

## Drop in your own game contract

Put a `.sol` file implementing `ICasinoGameV2` into `blockchain/contracts/` (import the interface
...
The simulator's Solidity sources live in `blockchain/contracts/` and compile with
`bun run blockchain:compile-contracts` (solc, `viaIR`), which regenerates the checked-in
`src/components/casino-simulator/local-node/artifacts.ts`. Only needed if you change the `.sol`
sources.
